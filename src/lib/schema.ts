...
// class defs, reserved class names
class ClassDefinitionNamesPass extends NodeVisitor {
    entities: ClassDef[] = []

    visitModule(node: any) {
        for (const stmt in node.body) {
            if (stmt.nodeType != "ClassDef") {
                throw SemaException()
            }

            if (stmt.name in RESERVED_CLASS_NAMES) {
                throw SemaException()
            }

            this.entities.push(stmt as ClassDef)
        }
    }
}

// inside entity fields:
// 1) no reserved identifiers
// 2) no repeating names
// 3)
class FieldNamesDefinitionPass {

}


function parseSource(source: string): Schema {
    try {
        const module = parse(source)
        const schema: Schema = {}


        const classdefs = new ClassDefinitionPass()
        classdefs.visit(module)
    }
}

export function parseSchemaSource(source: string): ValidatedSchema {
    let raw: unknown
    ...
    const entities = raw.entities.map((candidate, entityIndex) => {
        ...
        if (typeof candidate.tablename !== 'string') {
            throw new Error(`Entity ${candidate.name} must have a tablename.`)
        }
